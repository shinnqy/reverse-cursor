...
`)
    ...
    updateVisibleSuggestionText(e, t, s, n, r, o) {
      let a = s.startLineNumber,
        l = 1,
        c = []
      for (const u of t) {
        const d = u.value.split(o),
          g = a + d.length - 1,
          p = d.length > 1 ? d[d.length - 1].length + 1 : l + u.value.length
        u.added === !0
          ? c.push({ range: new G(a, l, a, l), text: u.value })
          : u.removed === !0 && c.push({ range: new G(a, l, g, p), text: "" }),
          u.added !== !0 && ((l = p), (a = g))
      }
      const h = tdi(c, n)
      if (
        (n.pushEditOperations([], c, () => null, r),
        n.pushStackElement(),
        c.every(
          (u) =>
            u.range.endLineNumber === u.range.startLineNumber &&
            u.range.endColumn === u.range.startColumn,
        ))
      ) {
        if (
...
`)
          ...
          e.setPosition(
            new Me(
              ...
              h.range.startColumn + h.text.length,
            ),
          )
        else if (c.length === 1) {
          const u = e.getPosition()
          if (u === null) return
          const d = G.fromPositions(
            u,
            new Me(h.range.startLineNumber, h.range.startColumn),
          )
          if (n.getValueInRange(d).trim().length === 0) {
            const g = _fo(h)
            e.setPosition(g)
          }
        }
    ...
    updateSuggestionGreenHighlights(e, t, s, n = !0) {
      const r = F_(e, t)
      if (r === null) return
      const { greenRanges: o } = OFt(s, r, "post-change")
      this.getCurrentSuggestion() &&
        (this.updateSuggestionStateHalfSilently({ greens: o }),
        this.Ob().cppAutoImportEnabled && this.Ib.handleNewGreens(e, o),
        n &&
    ...
    async makeChangesOnAcceptCppSuggestion({
      ...
      let l = F_(e, s)
      if (!l) return { type: Sp.NotAccepted }
      let c = s.originalText ?? ""
      const h = s.replaceText ?? "",
        { changes: u } = await this.vb.wordDiff(c, h)
      if (a) return { type: Sp.NotAccepted }
      const d = async (p) => {
          this.updateVisibleSuggestionText(n, u, p, e, s.undoRedoGroup, t)
          const { wordDiffs: m, charDiffs: b } = ZXe(
              s.originalText ?? "",
              h,
              t,
            ),
            w = this.Q.get(s.decorationId) === k7.PreviewBox ? m : b
          return (
            this.updateSuggestionGreenHighlights(e, s, w),
        ...
        g = async (p) => {
          const m = this.makeEditsForNextWordAccept({
            model: e,
            suggestion: s,
            editor: n,
            changes: u,
          })
          if (m.type === Sp.NotAccepted) return m
          if (m.type === Sp.AcceptedAll) return await d(p)
          {
            this.updateSuggestionState({
              suggestionIsCurrentlyHidden: !0,
              hasBeenSeen: !0,
            })
            const b = e.getValueInRange(p),
              { wordDiffs: y, charDiffs: w } = ZXe(s.originalText ?? "", b, t),
              S = this.Q.get(s.decorationId) === k7.PreviewBox ? y : w
            return this.updateSuggestionGreenHighlights(e, s, S), m
          }
        }
      return o ? await g(l) : await d(l)
    }
    increaseCppUsage() {
      this.hb.setApplicationUserPersistentStorage(
        "cppShown",
