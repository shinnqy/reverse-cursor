...
class _A {
  async immediatelyFireCppWithSpecifiedPosition({
    ...
    const cppConfig = this.Ob().cppConfig
    ...
    try {
      ...
      return (
        oa(`[Cpp] Bad Case: Error in getPartialCppRequest: ${error}`),
        { success: false }
      )
    }
    ...
    const C = this.S;
    const stream = this.streamCpp(abortController, C, generationUUID)
    if (stream == null)
      return (
    ...
    let modelOutputText;
    let fusedCursorPrediction = null;
    let fullText;
    let rangeToReplace;
    let modelInfo;
    let textToReplace;
    ...
    if (((this.cb = predictionId), isFusedCursorPredictionModel)) {
      ...
      const { firstChunk: firstChunkValue, fullText: fullTextValue } = await predictionStream.text;
      ...
      this.Ob().chunkedStreamingEnabledV2 === !0
        ? ([modelOutputText, fullText] = await handleChunkedStream(streamIterator, textToReplace, (position.lineNumber ?? 1) - rangeToReplace.startLineNumber))
        : (modelOutputText = await consumeRemainingStream(streamIterator))
    const range = new Hu({
      startLineNumber: rangeToReplace.startLineNumber,
      startColumn: 1,
    ...
    const z = model.getEOL();
    const K = this.getNotebookCellInfo(model, editor, z);
    const Q = await this.Fb.isValidCppCase({
        model: model,
        modelOutputText: modelOutputText,
    ...
    const recordFinishedGeneration = (Ee) => {
    ...
    if (!Q.valid) {
      const Ee = await fusedCursorPrediction
    ...
    ;(modelOutputText = Q.modelOutputText),
      fusedCursorPrediction?.then((Ee) => (this.uponFusedCursorPredictionReady(predictionId, Ee), !0))
    const suggestion = this.Gb.createUnseenSuggestion({
      model: modelSnapshot,
      diffText: modelOutputText,
      startingRange: range,
      eol: z,
      requestId: generationUUID,
      ...
      fullText !== void 0 && isSuggestionDisplayed && !abortController.signal.aborted
        ? fullText.then((Ee) => {
            abortController.signal.aborted ||
              ...
              this.generateFollowupSuggestion(editor, Ee, model, suggestion, position, z, predictionId),
              (this.R = this.R.filter((ke) => ke.generationUUID !== generationUUID)))
          })
        : fullText === void 0 &&
          (recordFinishedGeneration(modelOutputText), (this.R = this.R.filter((Ee) => Ee.generationUUID !== generationUUID))),
